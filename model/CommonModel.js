...
const getBlockList = (ardb_id, block_id=0) => {
    return new Promise(async (resolve, reject) => {
        try{
            ...
            const res_dt = await F_Select(0, select, table_name, whr, `ORDER BY block_name ASC`, 1)
            resolve(res_dt);
        }catch(err){
    ...
    return new Promise(async (resolve, reject) => {
        try {
            let select = 'a.*, b.block_name',
                table_name = 'v_service_area a, v_block b',
            ...
            const res_dt = await F_Select(0, select, table_name, whr, `ORDER BY a.service_area_name ASC`, 1)
            resolve(res_dt);
        } catch (err) {
            resolve({ suc: 0, msg: err })
        }
...
const getVillageList = (ardb_id, block_id, service_area_id, vill_id = 0) => {
    return new Promise(async (resolve, reject) => {
        try {
            let service_area = Array.isArray(service_area_id) ? service_area_id.map(dt => dt.toString()).join(',') : service_area_id
            if (service_area != 'all'){
                let select = 'a.*, b.service_area_name, c.block_name',
                    table_name = 'v_vill a, v_service_area b, v_block c',
                    whr = `a.ardb_id=b.ardb_id AND a.block_id=b.block_id AND a.service_area_id=b.service_area_id AND a.ardb_id=c.ardb_id AND a.block_id=c.block_id AND a.ardb_id = ${ardb_id} AND a.block_id = '${block_id}' AND a.service_area_id IN(${service_area}) ${vill_id > 0 ? `AND a.vill_id = '${vill_id}'` : ''}`;
                const res_dt = await F_Select(0, select, table_name, whr, `ORDER BY a.vill_name ASC`, 1)
                resolve(res_dt);
            }else{
                resolve({suc: 1, msg: []});
            }
